...
const AdminDashboard: React.FC = () => {
  ...
  const [currentView, setCurrentView] = useState<'dashboard' | 'items' | 'add' | 'edit' | 'categories' | 'payments' | 'settings' | 'orders' | 'inventory' | 'customers' | 'user-accounts'>('dashboard');
  ...
  if (currentView === 'user-accounts') {
    return <UserAccountManager onBack={() => setCurrentView('dashboard')} />;
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-noto font-semibold text-black">RR's Chicken Commissary Admin</h1>
            </div>
            <div className="flex items-center space-x-4">
              <a
                href="/"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={() => setCurrentView('user-accounts')}
                className="w-full flex items-center space-x-3 p-3 text-left hover:bg-gray-50 rounded-lg transition-colors duration-200 border-l-4 border-red-500 bg-red-50"
              >
                <Key className="h-5 w-5 text-red-600" />
                <span className="font-medium text-red-900">Manage User Accounts</span>
              </button>
              <button
