...
import { ArrowLeft, CheckCircle, Clock, XCircle, RefreshCw, ChevronDown, Search, Image as ImageIcon, Download, Calendar, DollarSign, FileText } from 'lucide-react';
    ...
    const filtered = useMemo(() => {
        ...
        const searched = q.length === 0
            ...
            : dateFiltered.filter(o =>
                ...
                (o.branch_name || '').toLowerCase().includes(q) // Added branch name to search
            );
    ...
    const exportToCSV = () => {
        ...
        try {
            ...
            const headers = [
                ...
                'OrderDateandTime',
                'ServiceType',
                'Branch', // Added Branch to CSV headers
                'remarks'
            ];
            ...
            const rows = completedOrders.map(order => {
                return [
                    ...
                    order.branch_name || 'N/A', // Added Branch to CSV rows
                    order.notes || 'N/A'
                ];
            });
    ...
    const handleDownloadInvoice = (order: OrderWithItems) => {
        try {
            ...
            const tableRows = order.order_items.map((item, index) => {
                ...
                const addons = item.add_ons && item.add_ons.length > 0
                        ...
                        <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
                            <div className="flex-1 relative">
                                ...
                                <input
                                    ...
                                    placeholder="Search orders by name, phone, ID, address, branch"
                                    ...
                                    <tbody className="divide-y divide-gray-200">
                                        {filtered.map((order) => (
                                            <tr key={order.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-5 py-4">
                                                    <div className="font-medium text-black px-3 py-1 bg-red-100 rounded text-xs inline-block mb-1">
                                                        {order.branch_name || 'No Branch'}
                                                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <div className="space-y-2 text-sm">
                                        ...
                                        <p><strong>Branch:</strong> {selectedOrder.branch_name || 'N/A'}</p>
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex-1">
                                                    ...
                                                    {item.menu_items?.description && (
                                                        <div className="text-xs text-gray-500 mt-0.5 italic">{item.menu_items.description}</div>
                                                    )}
                                                    {item.variation && (
